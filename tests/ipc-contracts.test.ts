...
describe('ipc contracts', () => {
  ...
  it('rejects variant creation with empty products', () => {
    expect(() => parseRunVariantCreationInput([], { themeId: 1, sizes: [{ key: 'S', themeValueId: 10, price: 1000 }] }, false)).toThrow(
      /products must be a non-empty array/i,
    );
  });

  it('rejects variant creation with invalid theme', () => {
    expect(() =>
      parseRunVariantCreationInput([{ productId: 1, productTitle: 'product' }], { themeId: 0, sizes: [{ key: 'S', themeValueId: 10, price: 1000 }] }, false),
    ).toThrow(/config\.themeId must be a positive integer/i);
  });

  it('rejects variant creation with invalid size item', () => {
    expect(() =>
      parseRunVariantCreationInput([{ productId: 1, productTitle: 'product' }], { themeId: 1, sizes: [{ key: '', themeValueId: 10, price: 1000 }] }, false),

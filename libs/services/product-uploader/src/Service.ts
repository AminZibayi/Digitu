...
interface UploadCsvRecord {
  ...
  general_mefa_id: number;
  description: string;
  title_fa: string;
  title_en: string;
  attr_usage_type_ids: number[];
  attr_piece_count_select_ids: number[];
  attr_visual_feature_ids: number[];
  attr_design: string;
  attr_frame_type_ids: number[];
  attr_frame_material_ids: number[];
  attr_frame_color: string;
  attr_surface_guard_ids: number[];
  attr_extra_description: string;
  attr_resistance_ids: number[];
  attr_washing_method_ids: number[];
  attr_general_design_ids: number[];
  attr_frame_thickness_mm: string;
  attr_print_type_ids: number[];
  advantages: string[];
...
export class ProductUploaderService {
  ...
  public async runUpload(
    ...
    for (let i = 0; i < parsedRows.length; i++) {
      ...
      try {
        ...
        if (autoPublish) {
          logger.info(`Auto-publishing product ${productId}`);
  ...
  private parseRow(row: Record<string, string>, rowNumber: number): UploadCsvRecord {
    const getString = (key: string, fallback = '') => String(row[key] ?? fallback).trim();
  ...
  private async saveBasicInfo(product: UploadCsvRecord): Promise<number> {
    const data = await this.client.requestJson<{
      data?: { draft_product_id?: number; bind?: { draft_product_id?: number }; id?: number; is_valid?: boolean; errors?: unknown };
  ...
  private addArrayAttr(attributes: Array<{ id: number; value: string | number | number[] }>, id: number, value: number[]): void {
    if (value.length > 0) {
      attributes.push({ id, value });
    }
  }

  private addStringAttr(attributes: Array<{ id: number; value: string | number | number[] }>, id: number, value: string): void {
  ...
  private async saveAttributes(draftProductId: number, product: UploadCsvRecord): Promise<void> {
    const attributes: Array<{ id: number; value: string | number | number[] }> = [];
  ...
  private async saveTitle(draftProductId: number, product: UploadCsvRecord): Promise<void> {
  ...
  private async uploadImage(imagePath: string, slot: number): Promise<string> {
    ...
    const response = await this.client.requestJson<{ data?: { data?: { id?: string }; id?: string } }>(
  ...
  private async finalizeProduct(draftProductId: number, encryptedIds: string[]): Promise<number | null> {
    const [mainImage, ...rest] = encryptedIds;
    const response = await this.client.requestJson<{ data?: { data?: { product_id?: number }; product_id?: number } }>(
  ...
  public async publishProduct(productId: number): Promise<boolean> {
    try {
      const res = await this.client.requestJson<{ status: string; data?: any }>(`/product/publish/${productId}/`, {
        method: 'POST',
        body: {}
      });
      return res.status === 'ok';
    } catch (error: any) {
      logger.error({ productId, error: error.message }, 'Failed to auto-publish product');
      return false;
    }
  }
}
